...
pub const Game = struct {
    ...
    pub fn update(self: *Self, part2: bool) !void {
        if (part2) {
            self.world_buffer.items[0].items[0] = '#';
            self.world_buffer.items[0].items[self.world_buffer.items[0].items.len - 1] = '#';
            self.world_buffer.items[self.world_buffer.items[0].items.len - 1].items[0] = '#';
            self.world_buffer.items[self.world_buffer.items[0].items.len - 1].items[self.world_buffer.items[0].items.len - 1] = '#';
        }
        for (0..self.world.items.len) |i| {
            for (0..self.world.items[i].items.len) |j| {
                var num_neighbors: u8 = 0;
                if (i > 0) {
                    if (self.world_buffer.items[i - 1].items[j] == '#') {
                        num_neighbors += 1;
                    }
                    if (j > 0) {
                        if (self.world_buffer.items[i - 1].items[j - 1] == '#') {
                            num_neighbors += 1;
                        }
                    }
                    if (j < self.world_buffer.items[i - 1].items.len - 1) {
                        if (self.world_buffer.items[i - 1].items[j + 1] == '#') {
                            num_neighbors += 1;
                        }
                    }
                ...
                if (j > 0) {
                    if (self.world_buffer.items[i].items[j - 1] == '#') {
                        num_neighbors += 1;
                    }
                }
                if (j < self.world_buffer.items[i].items.len - 1) {
                    if (self.world_buffer.items[i].items[j + 1] == '#') {
                        num_neighbors += 1;
                    }
                }
                if (i < self.world_buffer.items.len - 1) {
                    if (self.world_buffer.items[i + 1].items[j] == '#') {
                        num_neighbors += 1;
                    }
                    if (j > 0) {
                ...
                if (self.world_buffer.items[i].items[j] == '#') {
                    if (num_neighbors == 2 or num_neighbors == 3) {
                        self.world.items[i].items[j] = '#';
                    } else {
                        self.world.items[i].items[j] = '.';
                    }
                } else {
        ...
        if (part2) {
            self.world.items[0].items[0] = '#';
            self.world.items[0].items[self.world.items[0].items.len - 1] = '#';
            self.world.items[self.world.items[0].items.len - 1].items[0] = '#';
            self.world.items[self.world.items[0].items.len - 1].items[self.world.items[0].items.len - 1] = '#';
        }
        for (0..self.world.items.len) |i| {
            for (0..self.world.items[i].items.len) |j| {
                self.world_buffer.items[i].items[j] = self.world.items[i].items[j];
            }
        }
    }

    pub fn count_lights(self: *Self) !u32 {
